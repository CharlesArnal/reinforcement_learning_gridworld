...
for s = 1:( paramSet.rowCount * paramSet.colCount )
    ...
    if ~isbad,
        ...
        r = rc(1); c = rc(2);
        ang = thetaLUT( Popt( s ) );
        plot( c , r , 'co' ,"color","red",'LineWidth',1.5);
        plot( [ c (c + .25*cos( ang )) ], ...
              [ r (r - .25*sin( ang )) ], ...
               'c',"color","red" ,'LineWidth',1.5);
    end
end
    
% plot start and goal
plot( paramSet.goalState(2) , paramSet.goalState(1) , 'rp', 'markersize',20, 'LineWidth',1.5)
plot( paramSet.startState(2) , paramSet.startState(1) , 'rs', 'markersize',20,'LineWidth',1.5 )
